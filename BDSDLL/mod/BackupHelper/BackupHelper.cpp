...
namespace mod {
	void BackupHelper::copyFiles(const std::string &worldName,
		...
		level->forEachPlayer([&](Player *player) {
			sendMessage(player, myString(u8"°ÏeSaving£Æ£Æ£Æ from ", playerName));
		});
		...
		if(ec.value() != 0) {
			level->forEachPlayer([&](Player *player) {
				sendMessage(player, u8"°ÏcFailed to copy save files!\n" + ec.message());
			});
		...
		for(auto &file : files) {
			...
			if(hSaveFile == INVALID_HANDLE_VALUE || !SetFilePointerEx(hSaveFile, pos, &curPos, FILE_BEGIN)
			   || !SetEndOfFile(hSaveFile)) {
				level->forEachPlayer([&](Player *player) {
					sendMessage(player, u8"°ÏcFailed to truncate " + toFile + "!\n ");
				});
				FailEnd(GetLastError());
				return;
			}
	...
	void BackupHelper::listBackups(int pag = 1) {
		...
		if(backList.empty()) {
			sendMessage(player, myString(u8"°ÏcError£∫no backups"));
			isWorking = false;
			return;
		}
	...
	void BackupHelper::removeBackup(int slot) {
		...
		if(backList.empty()) {
			sendMessage(player,
						myString(u8"°ÏcError£∫no backups"));
			isWorking = false;
			return;
		} else if(slot > backList.size() || slot <= 0) {
			sendMessage(player,
						myString(u8"°ÏcError£∫max slot is ", backList.size()));
			isWorking = false;
			return;
		}

		level->forEachPlayer([&](Player *player) {
			sendMessage(player, myString(u8"°ÏeRemoving£Æ£Æ£Æ from ", playerName));
		});
	...
	void BackupHelper::restoreBackup(int slot) {
		...
		level->forEachPlayer([&](Player *player) {
			sendMessage(player, myString(u8"°ÏeRestoring£Æ£Æ£Æ from ", playerName));
		});
